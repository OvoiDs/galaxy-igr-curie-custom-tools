#!/usr/bin/Rscript
# Author: Thibault Dayris This is free and unencumbered software released into the public domain.
#
...
#
# For more information, please refer to <http://unlicense.org/> Libraries
...
parser <- ArgumentParser(
  ...
  metavar = "PATH",
  type = "character",
  help = "Path to report markdown file" ); parser$add_argument(
  "counts",
  metavar = "PATH",
  type = "character",
  help = "Path to counts.zip file" ); parser$add_argument(
  "-w", "--workdir",
  metavar = "PATH",
  type = "character",
...
opt <- parser$parse_args(); mc.cores <- opt$threads; opt$design <- as.character(opt$design); opt$tr2gene <- as.character(opt$tr2gene); opt$counts <- 
as.character(opt$counts); opt$gene_analysis <- as.logical(opt$gene_analysis); 
opt$output_table <- file.path(
  opt$workdir,
  paste0(opt$output, ".tsv") ); opt$output_rds <- file.path(
  opt$workdir,
  paste0(opt$output, ".rds") ); opt$output_exp <- file.path(
  opt$workdir,
  paste0(opt$output, ".expression.tsv") ); opt$args_out <- file.path(
  opt$workdir,
  paste0(opt$output, ".arguments.rds") ); opt$output_html <- file.path(
  opt$workdir,
...
meta <- read.table(opt$design, header = TRUE, sep = "\t"); meta$path <- as.character(meta$path); unzip(
  opt$counts,
  unzip="unzip", exdir="./counts" );
path2get=list.files(path=paste0(getwd(), "/counts"), pattern="abundance.h5", recursive=T, full.names=T)[[1]]
meta$path=dirname(path2get);
message(meta$path);
message("Metadata loaded");
# Prepare Sleuth run target_mapping is NULL by default in Sleuth
...
so <- sleuth_fit(so); message("Sleuth model fitted");
# The intercept only model
so <- sleuth_fit(so, ~1, "reduced"); message("Sleuth intercept fitted");
# Perform Wald test to get diff exp
tmp <- sort(meta$test); ref_cond <- paste0("test", tmp[length(meta$test)]); so <- sleuth_wt(so, which_beta = ref_cond); message("Wald Test over");
# Save results
write.table(
    ...
    quote = FALSE,
    row.names = FALSE,
    sep = "\t" ); message("Sleuth Result table saved");
# Geathering transcripts expression
quant <- sleuth::kallisto_table(so);
# Reshaping the dataframe
reduced_quant <- quant[c("target_id", "sample", "tpm")]; re_ordered <- tidyr::spread(reduced_quant, sample, tpm);
# Defining target_id as index column
rownames(re_ordered) <- re_ordered$target_id; re_ordered$target_id <- NULL;
...
write.table(
  ...
  quote = FALSE,
  row.names = TRUE,
  sep = "\t" ); message("Transcripts expression table saved");
# Save results table
write.table(
    sleuth_results(so, ref_cond),
    file = opt$output_table,
    ...
    row.names = FALSE,
    sep = "\t" ); message("Sleuth Result table saved");
# Saving RDS, as pairwise scatter plot may be very large for huge experimental designs.
sleuth::sleuth_save(so, file = opt$output_rds); message("RDS saved"); rmarkdown::render(
  opt$report,
  output_file = opt$output_html,
  params = list(sleuth = so, ref_cond = ref_cond) ); message("Report created"); message("Process is now over"); q(status = 0);
