...
parser.add_argument('-p', '--patient', dest='patient', type=str, required=True, help='Please give as input the patient tsv file')
...
parser.add_argument('-a', '--annotator', dest='annotator', type=str, required=True, default=None, help="Name of the annotator")
parser.add_argument('-k', dest='k', type=str, required=True, help="Dump the patient database")
...
if args.mode == "prepare":
    ...
    # We also need to add everything to the patient database
    ...
    try:
        ...
        panel = 31
    ann = str(args.annotator)
    pathos = ti.getIDByColname('Manual_Var_Classif')
    date = time.strftime('%d/%m/%Y')

    l_extract = lambda x: [str(x[chr]), str(int(x[pos]) - len(x[ref]) + 1), str(x[ref]), str(x[alt]), str(x[panel]), ann, str(x[pathos]), str(date) + '\n']

    database = read_database(args.database).split('\n')
    #print len(database)
    with open(args.patient, "a") as patienth:
        for line in ti.getLineIter():
            print "appending: {}".format(line)
            lineTab = []
            if l_extract(line)[6] in ['VP', 'VPI', 'VNP']:
                for k,v in sorted(line.iteritems()):
                    lineTab.append(str(v))
                lineTab.append('\n')
                patienth.write('\t'.join(lineTab))
            else:
                print 'Not appending {} to patient database'.format(line)
    ...
    if args.output:
        ...
        with open(args.output, "w") as outputh:
            for line in a:
                outputh.write(line)
    if args.k:
        ka = open(args.patient, "r")
        pat = ka.readlines()
        ka.close()
        with open(args.k, "w") as kh:
            for line in pat:
                kh.write(line)
else:
    pass
