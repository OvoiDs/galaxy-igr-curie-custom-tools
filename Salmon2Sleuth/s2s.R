...
parser <- ArgumentParser(
  description="Prepares a Sleuth process from Salmon files"
);
...
design <- data.frame(
  sample=args$names,
  path=sapply(args$salmon_paths, function(path) paste0(sub("^([^.]*).*", "\\1", path), "_dir/output")),
  test=args$conditions
);
print(design);

unzipped_name <- sapply(
  args$salmon_paths,
  function(path) paste0(sub("^([^.]*).*", "\\1", path), "_dir/output")
);
print(unzipped_name);

tmp <- sapply(
  args$salmon_paths,
  function(path) unzip(zipfile=path, unzip="unzip", exdir=paste0(sub("^([^.]*).*", "\\1", path), "_dir/"))
);

tmp <- sapply(
  unzipped_name,
  function(path) prepare_fish_for_sleuth(path)
);

zip(
  paste0(args$output, ".zip"),
  files=unzipped_name,
  zip="zip"
);
