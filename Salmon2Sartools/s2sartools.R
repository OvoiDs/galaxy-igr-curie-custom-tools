...
parser <- ArgumentParser(
  description="Prepares a SArtools process from Salmon files"
);
...
args$salmon_paths <- sapply(
  args$salmon_paths,
  function(path) as.character(path)
);

args$transcript_2_gene <- as.character(args$transcript_2_gene);
message("Command line parsed");
...
save_columns(round_df(agcounts));
